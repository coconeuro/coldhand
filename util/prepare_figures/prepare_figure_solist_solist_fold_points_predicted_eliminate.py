...
dv = 'regquality'
cols = ['subject',  dv, var_valid, 'prev_won', 'prev_lost', 'prev_fold', f'tdiff_{var_valid[6:]}', 'rule_kurze', 'game_type', 'cur_pos']
df = pd.read_parquet(os.path.join(data_root_fold_succeeding.replace('succeeding', 'eliminate'), f'{condition}.parquet'), columns=cols, engine='fastparquet')
