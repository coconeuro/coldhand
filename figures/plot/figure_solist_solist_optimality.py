...
plt.xticks([0, 1], ['Post-\nsuccess', 'Post-\nfailure'])
plt.xlim(-0.5, 1.5)
plt.ylim(97.39, 97.464)
plt.yticks(np.arange(97.40, 97.461, 0.02))
...
plt.text(-0.31, 1.055, 'Scope:', transform=plt.gca().transAxes, fontsize=11, fontweight='bold', color='#222', clip_on=False, bbox=dict(facecolor='#eee', ec='none', pad=3.25))
plt.text(0.02, 1.055, 'declared solo contracts', transform=plt.gca().transAxes, fontsize=11, color='#222', clip_on=False, bbox=dict(facecolor='#eee', ec='none', pad=3.25))
# plt.ylabel(f'Optimality (%)')
...
# plt.ylabel(f'Optimality', labelpad=19)
# plt.text(0.15, 0.48, '[factual − predicted points]', transform=plt.gcf().transFigure, fontsize=12, ha='center', va='center', rotation=90)
