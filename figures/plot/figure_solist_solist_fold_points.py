...
text_p = '***' if ff.loc[var_lost].p < 0.001 else ('*' if ff.loc[var_lost].p < 0.01 else ('*' if ff.loc[var_lost].p < 0.05 else ('o' if ff.loc[var_lost].p < 0.1 else 'ns')))
...
plt.xlim(-0.5, 2.5)
plt.ylim(73.3, 74.05)
# plt.text(-0.13, 1.055, ' Scope:', transform=plt.gca().transAxes, fontsize=10, fontweight='bold', color='#222', clip_on=False, bbox=dict(facecolor='#eee', ec='none', pad=3.25))
# plt.text(0.08, 1.055, 'FollowingSoloistGames ', transform=plt.gca().transAxes, fontsize=10, color='#222', clip_on=False, bbox=dict(facecolor='#eee', ec='none', pad=3.25))
plt.text(-0.15, 1.055, 'Scope:', transform=plt.gca().transAxes, fontsize=11, fontweight='bold', color='#222', clip_on=False, bbox=dict(facecolor='#eee', ec='none', pad=3.25))
plt.text(0.14, 1.055, 'declared solo contracts', transform=plt.gca().transAxes, fontsize=11, color='#222', clip_on=False, bbox=dict(facecolor='#eee', ec='none', pad=3.25))
plt.ylabel(f'Points')
