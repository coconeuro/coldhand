...
text_p = '***' if ff.loc[var_won].p < 0.001 else ('**' if ff.loc[var_won].p < 0.01 else ('*' if ff.loc[var_won].p < 0.05 else ('o' if ff.loc[var_won].p < 0.1 else 'ns')))
...
text_p = '***' if ff.loc[var_lost].p < 0.001 else ('**' if ff.loc[var_lost].p < 0.01 else ('*' if ff.loc[var_lost].p < 0.05 else ('*' if ff.loc[var_lost].p < 0.1 else 'ns')))
...
plt.xlim(-0.5, 2.5)
# plt.ylim(97.385, 97.465)
plt.ylim(97.38, 97.465)
plt.yticks(np.arange(97.38, 97.461, 0.02))
# plt.text(-0.19, 1.055, ' Scope:', transform=plt.gca().transAxes, fontsize=10, fontweight='bold', color='#222', clip_on=False, bbox=dict(facecolor='#eee', ec='none', pad=3.25))
# plt.text(0.1, 1.055, 'following soloist games ', transform=plt.gca().transAxes, fontsize=10, color='#222', clip_on=False, bbox=dict(facecolor='#eee', ec='none', pad=3.25))
plt.text(-0.29, 1.055, 'Scope:', transform=plt.gca().transAxes, fontsize=11, fontweight='bold', color='#222', clip_on=False, bbox=dict(facecolor='#eee', ec='none', pad=3.25))
plt.text(0.03, 1.055, 'declared solo contracts', transform=plt.gca().transAxes, fontsize=11, color='#222', clip_on=False, bbox=dict(facecolor='#eee', ec='none', pad=3.25))
...
# plt.title('Solo player')
# plt.text(0.14, 0.48, '[factual − predicted points]', transform=plt.gcf().transFigure, fontsize=12, ha='center', va='center', rotation=90)
plt.grid(axis='y', color=colors['grid'])
plt.text(0.02, 0.87, 'E', transform=plt.gcf().transFigure, fontsize=22)

plt.gca().get_xticklabels()[0].set_transform(plt.gca().get_xticklabels()[0].get_transform() + ScaledTranslation(-0.08, 0, plt.gcf().dpi_scale_trans))
plt.gca().get_xticklabels()[1].set_transform(plt.gca().get_xticklabels()[1].get_transform() + ScaledTranslation(0.03, 0, plt.gcf().dpi_scale_trans))
