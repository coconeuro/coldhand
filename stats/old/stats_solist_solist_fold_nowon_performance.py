import os
import pandas as pd
from coco_hothand.config import data_root_fold_nowon
...
dv = 'performance'
cols = [
    'subject',  'player_augen', 'regquality', var_valid, 'tdiff_new_max2', 'prev_lost', 'prev_won', 'prev_fold',
    'rule_kurze', 'game_type', 'cur_pos'
]
df = pd.read_parquet(os.path.join(data_root_fold_nowon, f'{condition}.parquet'), columns=cols, engine='fastparquet')
