import os
import pandas as pd
from coco_hothand.config import data_root_succeeding
...
df = pd.read_parquet(os.path.join(data_root_succeeding, f'{condition}.parquet'), columns=cols, engine='fastparquet')
df = df[df[var_valid] & (df.tdiff_new_max2 < 900)].astype(dict(prev_won='int', rule_kurze='int'))
...
if reload:
    regs = [
        'prev_won', 'rule_kurze', 'C(game_type)', 'C(cur_pos)'
    ]
    patsy = f"{dv} ~ {' + '.join(regs)}"
    model = Lmer(f"{patsy} + (1|subject)", data=df[[dv, 'subject'] + [reg.replace('C(', '').replace(')', '') for reg in regs]], family='binomial')
    # model = Lmer(f"{patsy} + (1|subject)", data=df[[dv, 'subject'] + [reg.replace('C(', '').replace(')', '') for reg in regs]])

    print(f"[{datetime.now().strftime('%H:%M:%S')}] Performing Lmer analysis: {patsy.replace(patsy[:patsy.index(' ~')], f'logit({dv})')}")
...
print(f"CI95 = [{100*coefs.iloc[1]['2.5_ci']:.2f}; {100*coefs.iloc[1]['97.5_ci']:.2f}]")
