...
condition = 'sologegner_all'
var_valid = 'valid_new_max3'
dv = 'solist'
var_ranS = 'prev_cond'
tmax = 900
cols = [
    'subject',  dv, var_valid, 'tdiff_new_max3_max', 'prev_lost', 'prev_won', 'prev_fold',
...
df = df[df[var_valid] & (df.tdiff_new_max3_max < tmax)]
...
print(f"Inrease of odds for losses relative to fold games: {100 * (np.exp(result.iloc[1]['Estimate']) - 1):.3f}%")
print(f"Inrease of odds for wins relative to fold games: {100 * (np.exp(result.iloc[2]['Estimate']) - 1):.3f}%")
