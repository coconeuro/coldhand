...
from stats.z_transform import std_z_transform_opponent, mean_z_transform_opponent
...
path_df = os.path.join(data_root_fold, f'{condition}.parquet')
df = pd.read_parquet(path_df, columns=cols, engine='fastparquet')
print(f'Loading file {path_df}')

df['opponent_optimality_ratio_z'] = df['opponent_optimality_ratio'] - mean_z_transform_opponent
df['opponent_optimality_ratio_z'] /= std_z_transform_opponent
