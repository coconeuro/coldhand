...
skill = pd.read_parquet('/home/matteo/schafkopf/pduser_skill_all.parquet', columns=['user_id', 'data_skill', 'data_score', 'data_score_full'], engine='fastparquet').rename(columns=dict(user_id='subject'))
df = df.merge(skill[['subject', 'data_skill', 'data_score', 'data_score_full']], on='subject', how='left', validate='many_to_one')
...
cov_check_means = [tuple([df[(df.cur_pos==pos) & (df.prev_cond == cond) & (df.rule_kurze == rk) & (df.game_type == gt)].groupby('subject').data_score_full.mean().mean() for cond in ('Fold', 'Lost', 'Won')]) for rk in (False, True) for gt in range(2, 6) for pos in range(4)]
cov_check_ses = [tuple([df[(df.cur_pos==pos) & (df.prev_cond == cond) & (df.rule_kurze == rk) & (df.game_type == gt)].groupby('subject').data_score_full.mean().sem() for cond in ('Fold', 'Lost', 'Won')]) for rk in (False, True) for gt in range(2, 6) for pos in range(4)]
import pickle
pickle.dump((cov_check_means, cov_check_ses), open(f"{__file__.split('/')[-1].replace('.py', '_covcheck.pickle')}", 'wb'))
