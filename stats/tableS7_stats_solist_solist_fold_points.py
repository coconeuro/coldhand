...
cols = [
    'id',
    'subject', dv, f'valid_{var_valid}', f'tdiff_{var_valid}_av', 'prev_lost', 'prev_won', 'prev_fold',
...
df = df[df[f'valid_{var_valid}'] & (df[f'tdiff_{var_valid}_av'] < tmax)]
...
print(f'N={len(df)}')
print(f'av time between failure and successes = {df[f'tdiff_{var_valid}_av'].mean():.1f} seconds')
